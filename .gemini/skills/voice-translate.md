...
27. **Microphone Recording Vibration Suppression (Android Known Issue):** On some Android devices, the physical vibration motor (`Vibration.vibrate()`) is automatically suppressed or muted by the Android OS/Audio HAL while an active audio recording session (microphone input) is starting or running. This is a system-level feedback protection mechanism to prevent motor humming from bleeding into the audio input. Standard and pattern-based vibrations (`Vibration.vibrate([0, 100])`) are currently not felt when pressing the mic, and must be resolved by exploring alternative haptic triggers or delays in a future task.
