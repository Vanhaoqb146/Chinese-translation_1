...
export default function SimultaneousPanel({
  ...
  const [silenceSeconds, setSilenceSeconds] = useState(() => Math.max(2, getSessionValue('silenceSeconds', 3)));
  ...
  const [micMode, setMicMode] = useState(() => getSessionValue('micMode', 'continuous'));
  const [autoTTS, setAutoTTS] = useState(() => getSessionValue('autoTTS', true));
  const [overlapListening, setOverlapListening] = useState(() => getSessionValue('overlapListening', false));
  ...
  useEffect(() => {
    try {
      sessionStorage.setItem('vt_setting_sim_provider', JSON.stringify(provider));
      sessionStorage.setItem('vt_setting_sim_silenceSeconds', JSON.stringify(silenceSeconds));
      sessionStorage.setItem('vt_setting_sim_srcVoice', JSON.stringify(srcVoice));
      sessionStorage.setItem('vt_setting_sim_tgtVoice', JSON.stringify(tgtVoice));
      sessionStorage.setItem('vt_setting_sim_autoDetect', JSON.stringify(autoDetect));
      ...
      sessionStorage.setItem('vt_setting_sim_fontSize', JSON.stringify(fontSize));
      ...
      sessionStorage.setItem('vt_setting_sim_overlapListening', JSON.stringify(overlapListening));
    } catch { /* ignore */ }
  }, [provider, silenceSeconds, srcVoice, tgtVoice, autoDetect, micMode, autoTTS, fontSize, muteSrc, muteTgt, overlapListening]);

  // Reset voices when provider changes
  useEffect(() => {
  ...
  return (
    <div className="conv-auto">
      ...
      {/* ============ HEADER BAR ============ */}
      <div className="conv-header-bar" style={{ background: 'linear-gradient(90deg, rgba(14,165,233,0.1) 0%, rgba(139,92,246,0.1) 100%)', borderBottom: '1px solid rgba(14,165,233,0.2)' }}>
        ...
        <div className="conv-header-center" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '2px' }}>
          <span style={{ fontSize: '13px', fontWeight: 700, color: '#0ea5e9', letterSpacing: '0.5px' }}>🎙️ DỊCH SONG SONG</span>
          {conv.queueLength > 0 && (
            <span style={{
              fontSize: '10px', fontWeight: 'bold', background: 'rgba(234,179,8,0.15)',
              color: '#fbbf24', border: '1px solid rgba(234,179,8,0.3)',
              padding: '1px 8px', borderRadius: '10px', animation: 'pulse 1.5s infinite'
            }}>
        ...
        <div className="conv-log-body" ref={logBodyRef}>
          {history.length === 0 && !interimText && (
            <div className="conv-empty">
              <div className="conv-empty-icon" style={{ textShadow: '0 0 15px rgba(14,165,233,0.4)', color: '#0ea5e9' }}>🎙️</div>
              <div style={{ fontWeight: 'bold', color: 'var(--text1)' }}>Chế độ Dịch song song (Simultaneous)</div>
              <div className="conv-empty-sub" style={{ maxWidth: '300px', margin: '8px auto', fontSize: '12px', lineHeight: '1.5' }}>
                Hệ thống thu âm liên tục không dừng. Khi bạn im lặng, câu nói sẽ tự động chuyển dịch tuần tự trên màn hình mà không ngắt mic!
              </div>
              <div style={{ marginTop: '12px', display: 'flex', justifyContent: 'center' }}>
                <span style={{ fontSize: '11px', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', padding: '4px 12px', borderRadius: '8px', color: 'var(--text3)' }}>
                  {!autoTTS ? '💡 Đã tắt giọng đọc - Micro mở 100% không rú âm!' : '💡 Đeo tai nghe nếu sử dụng chế độ Nghe đè!'}
                </span>
              </div>
            </div>
          )}
          ...
          {history.slice().reverse().map((h, index) => {
            ...
            return (
              <div key={`sim-${h.id}-${index}`} className={`chat-bubble-group ${alignment}`}>
                {/* Source utterance */}
                <div className="chat-bubble bubble-original" style={{ fontSize: `${fontSize}px`, borderLeft: isSource ? 'none' : '3px solid #8b5cf6' }}>
                  ...
                  <button
                    ...
                    onClick={() => handleReplay(h.source, findSttCode(h.fromLang), `src-${h.id}`)}
                    title={replayingId === `src-${h.id}` ? 'Dừng' : 'Nghe câu gốc'}
                  >{replayingId === `src-${h.id}` ? '🔇' : '🔊'}</button>
                </div>

                {/* Target translation */}
                <div className="chat-bubble bubble-translated" style={{ fontSize: `${fontSize}px`, borderLeft: isSource ? '3px solid #0ea5e9' : 'none' }}>
                  <span className="chat-bubble-flag">{getFlagForLang(h.toLang)}</span>
                  <span className="chat-bubble-text">{h.target}</span>
                  <button
                    className="chat-bubble-speaker"
                    onClick={() => handleReplay(h.target, findSttCode(h.toLang), `tgt-${h.id}`)}
          ...
          {interimText && (
            <div className="chat-bubble-interim" style={{ fontSize: `${fontSize}px` }}>
              <div className="chat-bubble-interim-text" style={{ borderLeft: '3px solid rgba(14,165,233,0.5)', background: 'rgba(14,165,233,0.05)' }}>
          ...
          <>
            <div className="fab-mic-group">
              <button
                ...
                style={{
                  background: conv.isListening ? 'linear-gradient(135deg, #ef4444 0%, #dc2626 100%)' : 'linear-gradient(135deg, #0ea5e9 0%, #8b5cf6 100%)',
                  boxShadow: conv.isListening ? '0 0 20px rgba(239,68,68,0.5)' : '0 0 20px rgba(14,165,233,0.3)'
                }}
              >
                ...
                {conv.isListening && convStatus === 'listening' && <span className="pulse-ring" style={{ borderColor: '#ef4444' }} />}
                {conv.isListening && convStatus === 'listening' && <span className="pulse-ring p2" style={{ borderColor: '#ef4444' }} />}
              </button>
              <div className="fab-mic-label" style={{ fontWeight: 'bold' }}>
                {conv.activeLang ? getFlagForLang(conv.activeLang) : '🌐'} Live Auto
              </div>
            </div>

            <div className="fab-status">
              <span className="fab-status-text" style={{ fontSize: '13px', fontWeight: 'bold' }}>
                ...
                {convStatus === 'speaking' && (
                  <button className="fab-stop-speaking-btn" onClick={handleStopSpeaking}>🔇 Tắt phát âm</button>
                )}
              </span>
          ...
          <>
            ...
            <div className="fab-mic-group">
              <button
                ...
                style={{
                  background: conv.activeLang === srcLang.translateCode ? 'linear-gradient(135deg, #ef4444 0%, #dc2626 100%)' : 'linear-gradient(135deg, #2563eb 0%, #1d4ed8 100%)',
                  boxShadow: conv.activeLang === srcLang.translateCode ? '0 0 15px rgba(239,68,68,0.4)' : '0 0 15px rgba(37,99,235,0.2)'
                }}
              >
                ...
                {conv.activeLang === srcLang.translateCode && convStatus === 'listening' && <span className="pulse-ring" style={{ borderColor: '#ef4444' }} />}
                {conv.activeLang === srcLang.translateCode && convStatus === 'listening' && <span className="pulse-ring p2" style={{ borderColor: '#ef4444' }} />}
              </button>
            ...
            <div className="fab-status">
              <span className="fab-status-text">
                {convStatus === 'idle' && (isHoldMode ? '👇 Nhấn giữ' : '👆 Chọn Mic để nói')}
            ...
            <div className="fab-mic-group">
              <button
                ...
                style={{
                  background: conv.activeLang === tgtLang.translateCode ? 'linear-gradient(135deg, #ef4444 0%, #dc2626 100%)' : 'linear-gradient(135deg, #8b5cf6 0%, #7c3aed 100%)',
                  boxShadow: conv.activeLang === tgtLang.translateCode ? '0 0 15px rgba(239,68,68,0.4)' : '0 0 15px rgba(139,92,246,0.2)'
                }}
              >
                <span className="fab-mic-btn-icon">
                ...
                {conv.activeLang === tgtLang.translateCode && convStatus === 'listening' && <span className="pulse-ring" style={{ borderColor: '#ef4444' }} />}
                {conv.activeLang === tgtLang.translateCode && convStatus === 'listening' && <span className="pulse-ring p2" style={{ borderColor: '#ef4444' }} />}
              </button>
          ...
          <div className="drawer-content">
            ...
            <div className="drawer-body">
              ...
              <div className="drawer-section">
                ...
                <div style={{
                  fontSize: '11px', color: autoTTS ? '#6b7280' : '#10b981', background: autoTTS ? 'rgba(255,255,255,0.02)' : 'rgba(16,185,129,0.06)',
                  borderRadius: 6, padding: '6px 10px', marginTop: '-4px', marginBottom: '8px', lineHeight: '1.4',
                  border: autoTTS ? 'none' : '1px solid rgba(16,185,129,0.15)'
                }}>
                  {autoTTS 
                ...
                {autoTTS && (
                  <>
                    <div className="drawer-row" style={{ borderTop: '1px solid rgba(255,255,255,0.05)', paddingTop: '10px' }}>
                      <label>🎧 Nghe đè khi phát (Dùng tai nghe)</label>
                      <div
                    ...
                    <div style={{
                      fontSize: '11px', color: '#eab308', background: 'rgba(234,179,8,0.06)',
                      borderRadius: 6, padding: '6px 10px', marginTop: '-4px', marginBottom: '8px', lineHeight: '1.4',
                      border: '1px solid rgba(234,179,8,0.15)'
                    }}>
                      ⚠️ Nghe đè cho phép mic thu âm tiếp trong lúc máy đang đọc. Khuyến nghị đeo tai nghe để tránh loa phát rú âm lặp ngược vào mic.
                    </div>
                  </>
                )}

                {/* Silence slider */}
                <div className="drawer-row" style={{ gap: 8, borderTop: '1px solid rgba(255,255,255,0.05)', paddingTop: '10px' }}>
              ...
              <div style={{
                ...
                ⚠️ Khuyên dùng khoảng im lặng 2s - 3s để dịch song song có trải nghiệm bám đuổi tốt nhất!
              </div>
            </div>
          </div>
        </>
